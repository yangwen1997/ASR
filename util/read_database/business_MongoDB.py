...
import numpy as np
import pandas as pd
from base_class.read_mongo import Read_From_Mongo
from property.config.path import data_path
...
class ReadBusiness(Read_From_Mongo):
    def __init__(self, db=mongo_DB):
        super(ReadBusiness, self).__init__(db=db)
        self.table_name = ''

    def read_from_mongo(self, table_name=None, column_list=None, ignore_column_list = None,filters=None):
        # 父类方法不满足时，再去重写父类方法
        return super(ReadBusiness, self).read_from_mongo(table_name=table_name, column_list=column_list,ignore_column_list = ignore_column_list,
                                                          filter_dict=filters)


def read_business():
    read = ReadBusiness(db=mongo_DB)
    ignore_column_list = []
    return read.read_from_mongo(table_name=mongo_busTable, filters={'p_code': "BUS_YT_DK"},
                                ignore_column_list=ignore_column_list)  # 读取客户评级属性
