...
class Tokenizer(object):
        ...
        with self.lock:
            ...
            if load_from_cache_fail:
                ...
                with wlock:
                    ...
                    print("self.FREQ, self.total = self.gen_pfdict(self.get_dict_file()) :")
                    print(self.FREQ, self.total)
                    print("==========================")
                    default_logger.debug(
    ...
    #TODO: 动态规划，计算最大概率的切分组合
    def calc(self, sentence, DAG, route):
        """
        登陆词部分  为什么采用从后往前这种方式计算呢？  因为，我们这个有向无环图的方向是从前向后指向，对于一个节点，我们只知道这个节点会指向后面哪些节点，但是我们很难直接知道有哪些前面的节点会指向这个节点。
        calc(sentence,DAG,0,route)#根据得分进行初步分词
        :return: route 最大概率的切分组合
        calc是一个自底向上的动态规划(重叠子问题、最优子结构)，它从sentence的最后一个字(N-1)开始倒序遍历sentence的字(idx)的方式，
            计算子句sentence[isdx~N-1]概率对数得分（这里利用DAG及历史计算结果route实现）。
            然后将概率对数得分最高的情况以（概率对数，词语最后一个字的位置）这样的tuple保存在route中。 
        """
        N = len(sentence)
        route[N] = (0, 0)
    ...
    # https://www.jb51.net/article/168807.htm
    ...
    def cut(self, sentence, cut_all=False, HMM=True):
        ...
        '''
        # 需要保证“不需要”已存在于self.FREQ
        ret = re.findall(r"[别|不]{1}\w{0,5}打\w{0,3}电话", sentence)
        if len(ret) > 0:
            for name in ret:
                sentence = sentence.replace(name, "不需要")
...
print("Just get jieba default dict.txt,Before add dgg.txt:: ")
print(get_dict_file())
# 按照get_dict_file的数据结构 添加顶呱呱的特殊数据  需要 27430 v  不要 14786 df  别 14132 r  别再打电话=不需要

print("After add dgg.txt: ")
