...
from center_three.voice_label.utils import Read_From_Mysql,Insert_Into_Mysql,table_data,MongoPool
...
class Get_Remarkdata(object):
    ...
    def __init__(self):
        ...
        self.mongopool = MongoPool(Mongouri)

    def get_basic_data(self, timestamp):
        mongo_conn = self.mongopool.get_conn()
        set_conn = self.mongopool.get_set(mongo_conn, MongoDataBase, MongoSet)
        now_time = self.table_data(timestamp)
        now_time_yesterday = pd.to_datetime(now_time) + Hour(24)
        now_time_yesterday = str(now_time_yesterday.date())
        query = {'status': 0, 'call_time': {'$lte': now_time_yesterday}}
        data_list = [i for i in set_conn.find(query).limit(1000)]
        basic_df = pd.DataFrame(data_list)
        self.mongopool.close(mongo_conn)
        if len(basic_df) != 0:
            return basic_df
        else:
            return pd.DataFrame()
    ...
    #     sql = "select * from {} where status = 0 and updated_at is null and call_time <= '{}' limit 1000".format(table_name, now_time_yesterday)
    ...
    def basic_match_post(self, df, timestamp):
        ...
        for value in np_array:
            ...
            if call_time != None:
                ...
                end_time = (pd.to_datetime(call_time) + Hour(24)).date().strftime('%Y-%m-%d %H:%M:%S').strip()
                value["remark_result"], value["status"], value["unstandard"], value["remark_id"] = self.get_post_data(
                    businessId, customerId, userid, start_time, end_time)
            else:
                value["status"] = 1
...
#                 end_time = (call_time + Hour(24)).date().strftime('%Y-%m-%d %H:%M:%S').strip()
...
# TODO 未验证
    def match_remark(self, data_df, regex_df):
        ...
        data_records = data_df.to_records(index=False)
        data_array = np.asarray(data_records)
        regex_records = regex_df.to_records(index=False)
        regex_array = np.asarray(regex_records)

        for data in data_array:
            content = data["remark_result"]
            sid_list = []
            ylid_list = []
            result_list = []
            label_list = []
            unstandard = data["unstandard"]
            if unstandard == 1:
                data["dmlabel"] = ["无备注"]
                data["sid"] = [0]
                data["ylid"] = [0]
            ...
            for regex in regex_array:
                ylid = regex["ylid"]
                sid = regex["sid"]
                question_word = regex["question_word"]
            ...
            if len(label_list) != 0:
                data["dmlabel"] = label_list
                data["sid"] = sid_list
                data["ylid"] = ylid_list
                data["remark_zresult"] = result_list
        ...
        data1 = pd.DataFrame({
            "_id": np.repeat(data1._id.values, data1.dmlabel.str.len()),
        ...
        data.drop("unstandard", axis=1, inplace=True)

        data.remark_result = data.remark_result.apply(lambda x: np.NaN if x == "" else x)
        data.remark_id = data.remark_id.apply(lambda x: np.NaN if x == "" else x)
        data.sid = data.sid.apply(lambda x: np.NaN if x == 0 else x)
        data.ylid = data.ylid.apply(lambda x: np.NaN if x == 0 else x)
        data.remark_zresult = data.remark_zresult.apply(lambda x: np.NaN if x == "" else x)
        data.dmlabel = data.dmlabel.apply(lambda x: np.NaN if x == "" else x)
        return data
